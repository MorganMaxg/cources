...
public class FileUtils {
    //批量文件上传
    public static String upload(String part, MultipartFile[] files, HttpServletRequest request) {
    ...
    //单文件上传
    public static String upload(String part, MultipartFile file, HttpServletRequest request) {
        JSONObject data = new JSONObject();
        try {
            File file1 = new File(request.getSession().getServletContext().getRealPath("/WEB-INF/files/" + part + "/")
                    , "" + System.currentTimeMillis()
