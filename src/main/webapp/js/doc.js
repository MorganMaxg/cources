$(function () {
    ...
    $header.load(fixPath('/docs/partial/navbar.html'), function () {
        ...
        $navbarCollapse.find('.nav > .nav-heading').click(function () {
            ...
            if ($nav.hasClass('collapsed')) {
                ...
                $nav.children('li:not(.nav-heading)').slideUp('fast', function () {
        ...
        $('#main').children('section').each(function () {
            ...
            if (lib) {
                if (itemInLib(standard, id, data.lib) >= 0) {
                    $header.children('h2').append(' <span class="label label-badge label-primary" title="姝ょ粍浠跺寘鍚湪鏍囧噯鐗堜腑鎻愪緵">ZUI</span>');
                }

                if (itemInLib(lite, id, data.lib) >= 0) {
                    $header.children('h2').append(' <span class="label label-badge label-info" title="姝ょ粍浠朵篃鍦ㄧ簿绠€鐗堜腑鎻愪緵">ZUI.LITE</span>');
                }

                if (itemInLib(separate, id, data.lib) >= 0) {
                    $header.children('h2').append(' <span class="label label-badge label-success" title="姝ょ粍浠跺湪lib鐩綍涓崟鐙彁渚�">LIB</span>');
                }

                if (lib.ver) {
                    $header.children('h2').append(' <span class="label label-badge" title="鏈€鏃╂彁渚涚増鏈负' + lib.ver + '">v' + lib.ver + '+</span>');
                }

                if (lib.thirdpart) {
                    $header.children('h2').append(' <a target="_blank" href="' + lib.website + '" class="label label-badge label-danger" title="杩欐槸涓€涓涓夋柟缁勪欢锛岀偣鍑昏闂綉绔�"><i class="icon-heart"></i>' + (lib.pver ? (' v' + lib.pver) : '') + '</a>');
                }

                if (!lib.src && lib.dpds) {
                    $header.children('h2').append(' <span class="label label-badge label-warning" title="姝ょ粍浠跺彲浠ユ媶鍒嗕娇鐢�"><i class="icon-cogs"></i></span>');
                }
            }
        });
        ...
        if ($buildTable.length) {
            ...
            for (var itemName in data.lib) {
                ...
                if (!item.src && item.dpds) {
                    ...
                    childComps = '鍚堝苟缁勪欢鍖呭惈锛�';
                    childComps += $.map(childList, getChildCompsList).join('銆�');
                }
    ...
    // $('#main > section[data-lite] > .page-header > h2').append(' <small class="label label-info" title="" data-original-title="姝ゅ唴瀹逛篃鍦ㄧ簿绠€鐗堜腑鎻愪緵">LITE</small>');
            ...
            K.create('textarea.kindeditor',
                {
                    ...
                    afterBlur: function () {
                        $('#content').prev('.ke-container').removeClass('focus');
                    },
                    afterFocus: function () {
                        $('#content').prev('.ke-container').addClass('focus');
                    }
                });
            ...
            K.create('textarea.kindeditorSimple',
                {
                    ...
                    afterBlur: function () {
                        $('#contentSimple').prev('.ke-container').removeClass('focus');
                    },
                    afterFocus: function () {
                        $('#contentSimple').prev('.ke-container').addClass('focus');
                    }
                });
            ...
            K.create('textarea.customStyle',
                {
                    ...
                    bodyClass: 'article-content',
                    afterBlur: function () {
                        $('#contentCustom').prev('.ke-container').removeClass('focus');
                    },
                    afterFocus: function () {
                        $('#contentCustom').prev('.ke-container').addClass('focus');
                    }
                });
        });
    }
